...
const SECTIONS={
  ...
  chores:['Huishouden','✓'], stock:['Voorraad','▣'], ideas:['Ideeën','♡'], home:['Woning','⌂'],
...
function render(){
  updateHeader();
  const map={today:renderToday,planning:renderPlanning,meals:renderMeals,groceries:renderGroceries,chores:renderChores,stock:renderStock,ideas:renderIdeas,home:renderHome,trips:renderTrips,imports:renderImports,settings:renderSettings};
...
function renderStock(){
  ...
  return `<section class="card notice good"><div class="card-head"><div><p class="eyebrow">Bodemprijzen</p><h2>Automatisch prijzen volgen</h2></div><button class="secondary" data-refresh-prices>↻ Prijzen vernieuwen</button></div><p>De app zoekt actuele aanbiedingen via de ingestelde prijsservice. Pas je zoekterm aan als de match niet exact genoeg is. PrijsProfeet is een aanbiedingenbron, geen volledig supermarktassortiment.</p></section>
...
function renderSettings(){
  ...
  return `<div class="settings-grid">
    ...
    <section class="card settings-card"><p class="eyebrow">Prijsservice</p><h2>PrijsProfeet via Supabase</h2><p>De browser praat met jouw Edge Function <code>grocery-prices</code>. Een eventuele PrijsProfeet API-key staat als Supabase secret en dus niet in deze app.</p><div class="button-row"><button class="secondary" data-refresh-prices>Prijsservice testen</button></div><p class="muted">De gratis bron bevat vooral aanbiedingen. Cross-retailer EAN-matching en officiële prijshistorie vereisen een passend PrijsProfeet-plan.</p></section>
