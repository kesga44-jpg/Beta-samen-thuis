...
### Optioneel: PrijsProfeet API-key

Zet een key nooit in `app.js`, GitHub of `index.html`.

Gebruik een Supabase secret:

```bash
npx supabase secrets set PRIJS_PROFEET_API_KEY=JOUW_KEY
npx supabase functions deploy grocery-prices --no-verify-jwt
```

Zonder key gebruikt de functie de publieke endpoints binnen hun publieke rate limits.
