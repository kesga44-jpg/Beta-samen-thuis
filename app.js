...
function render(){
  updateHeader();
  const map={today:renderToday,planning:renderPlanning,meals:renderMeals,groceries:renderGroceries,chores:renderChores,stock:renderStock,ideas:renderIdeas,home:renderHome,cars:renderCarsV17,data:renderDataSources,trips:renderTrips,imports:renderImports,settings:renderSettings};
...
function modal(html){
 let d=document.querySelector('#autoDialog');
 if(!d){d=document.createElement('dialog');d.id='autoDialog';d.className='dialog';document.body.appendChild(d);}
 d.innerHTML=`<div class="dialog-head"><div><p class="eyebrow">Auto</p><h2>Auto & brandstof</h2></div><button class="icon-btn" type="button" data-modal-close>×</button></div>${html}`;
 if(typeof d.showModal==='function')d.showModal();else d.setAttribute('open','');
}
function closeModal(){const d=document.querySelector('#autoDialog');if(!d)return;if(typeof d.close==='function'&&d.open)d.close();else d.removeAttribute('open');}
...
document.addEventListener('click',async e=>{
  ...
  if(e.target.closest('#addBtn')){ if(current==='cars'){if(autoTab==='vehicles')carDialog();else fuelDialog();return;} if(current==='trips'){ if(!store.data.tripFolders.length) return openFolderForm(); return openForm('trips'); } openForm(current); return; }
